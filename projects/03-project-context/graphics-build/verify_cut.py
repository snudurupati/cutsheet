...
def cut_to_source(cs, t):
    """Map a time on the edited timeline back to camera-clip seconds."""
    fps = cs["fps"]; acc = 0
    for s in cs["segments"]:
        n = s["endFrame"] - s["startFrame"]
        if acc / fps <= t < (acc + n) / fps:
            return (s["startFrame"] + round((t - acc / fps) * fps)) / fps, s["id"]
        acc += n
    return None, None
...
def main():
    ...
    if a.halves != "audio":
        ...
        for t in marks:
            ...
            frame_png(cam, st + 3.0, F, grade=a.grade or None)
            far = mean_abs_diff(A, F) or 99.0
            ratio = far / ds[best] if ds[best] else 0
            good = abs(best - W) <= 1 and ratio >= 1.25
            print(f"{t:9.2f} {st:10.2f} {sid:>6} {best-W:+6d} {ds[best]:7.3f} "
