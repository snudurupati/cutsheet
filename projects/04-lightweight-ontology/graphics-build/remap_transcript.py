...
Session-aware (04-lightweight-ontology is two recordings): a word is only ever
matched against segments of ITS OWN session, because both sessions have a 5:00.
...
def main():
    ...
    doc = {
        ...
        "sessions": [x["id"] for x in cs["sessions"]],
        "fps": fps,
        "totalFrames": total_frames,
        "totalSeconds": round(total_frames / fps, 3),
        "cutsheet": a.cutsheet,
        "_note": "word timings are on the EDITED timeline. sourceStart is the "
                 "original time on that word's own session camera clip, kept so a cue can be traced back.",
        "words": out_words,
    }
