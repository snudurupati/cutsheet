...
Gotchas baked in, every one of which ships a broken video silently:
  ...
  * 04-lightweight-ontology: layers are placed and enabled on FRAME NUMBERS from the
    cutsheet (startFrame/endFrame), never on seconds. A part at 20.767s whose first
    frame is 20.7667 was enabled one frame late and showed a face frame inside the
    takeover (technical QA, 2026-09-22). The output is capped at the cut's exact
    frame count.
"""
...
def main():
    ...
    TOTAL = round(cs["totalSeconds"] * a.fps)
    inputs = ["-i", a.base]
    chain = []
    cur = "0:v"
    n = 1
    ...
    if a.demo_scene:
        ...
        SF, EF = dj["enters"]["frame"], dj["leaves"]["frame"]
        chain.append(f"[{n}:v]setpts=PTS-STARTPTS+{SF}/(30*TB)[dm]")
        chain.append(f"[{cur}][dm]overlay=0:0:eof_action=pass:"
                     f"enable='between(n,{SF},{EF - 1})'[v{n}]")
        cur = f"v{n}"; n += 1
    ...
    for p in parts:
        ...
        built = p.get("builtBy")
        if built is None:
            sys.exit(f"{p['id']}: no builtBy in the cutsheet; every part must say how it is made")
        if built == "demo_scene":
            continue              # a punch-in inside the demo scene: no render by design
        if built == "push":
            f = os.path.join(a.renders, f"{p['id']}.mp4")
        if not os.path.exists(f):
            sys.exit(f"missing render for {p['id']} ({built}): {f}")
        comp = os.path.join(os.path.dirname(a.renders.rstrip("/")), "parts", p["id"], "index.html")
        if built != "push" and os.path.getmtime(f) < os.path.getmtime(comp):
            sys.exit(f"{p['id']}: render is OLDER than its composition; re-render before compositing")
        # A render shorter than its part runs out with eof_action=pass and the layer just
        # vanishes early; this is how a stale g21 dropped out 2.2s before its end
        # (2026-09-23). Its length must be the PART's, from the cut sheet.
        nf = int(subprocess.run(["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries",
                                 "stream=nb_frames", "-of", "csv=p=0", f],
                                capture_output=True, text=True, check=True).stdout.strip())
        if nf != p["endFrame"] - p["startFrame"]:
            sys.exit(f"{p['id']}: render has {nf} frames, the cut sheet part spans "
                     f"{p['endFrame'] - p['startFrame']}; rebuild and re-render it")
        inputs += ["-i", f]
        # setpts, never -ss. eof_action=pass, never repeat. Frames, never seconds.
        sf, ef = p["startFrame"], p["endFrame"]
        chain.append(f"[{n}:v]setpts=PTS-STARTPTS+{sf}/(30*TB)[p{n}]")
    ...
    cmd = ["ffmpeg", "-nostdin", "-y", "-v", "error", "-stats", *inputs,
           ...
           "-c:a", "pcm_s24le", "-map_chapters", "-1", "-frames:v", str(TOTAL), a.out]
