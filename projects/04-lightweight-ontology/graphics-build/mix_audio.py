...
def main():
    ...
    eq=p.get("voiceChain")
    if not eq:
        sys.exit("the plan has no voiceChain; the voice processing is part of the plan")
    ...
    # Aim 0.2 dB INSIDE the true-peak ceiling. On 04-lightweight-ontology the mix
    # landed at -1.4 dBTP against a -1.5 ceiling: loudnorm treats TP as a target, not
    # a wall, and the -1.0 dBFS sample limiter below never engaged. The delivery
    # target itself (and the report against it) is unchanged.
    LN = f"loudnorm=I={tgt['lufs']}:TP={tgt['truePeakDb'] - 0.2:.1f}:LRA=11"
