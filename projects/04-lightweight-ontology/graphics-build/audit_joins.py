...
def main():
    ...
    for x, y in zip(segs, segs[1:]):
        ...
        if last["session"] == first["session"]:
            gone = [w for w in src if w["session"] == last["session"]
                    and last["sourceStart"] < w["start"] < first["sourceStart"]]
        else:   # a session boundary: the tail of one and the head of the other
            gone = ([w for w in src if w["session"] == last["session"] and w["start"] > last["sourceStart"]] +
                    [w for w in src if w["session"] == first["session"] and w["start"] < first["sourceStart"]])
        ...
        if flags:
            flagged += 1
        t = last["end"]
        where = (f"   removed {first['sourceStart']-last['sourceStart']:.1f}s"
                 if last["session"] == first["session"] else
                 f"   SESSION {last['session'].upper()} -> {first['session'].upper()}")
        print(f"[{x['id']} -> {y['id']}]  cut {int(t)//60}:{int(t)%60:04.1f}{where}"
