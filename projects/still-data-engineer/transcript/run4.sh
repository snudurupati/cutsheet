...
set -euo pipefail
cd "$(dirname "$0")/.."
echo "### re-rendering the 8 changed parts"
for p in g005 g006 g010 g011 g012 g020 g023 g024; do rm -f outputs/.cache/parts/$p.mov; done
ONLY=g005,g006,g010,g011,g012,g020,g023,g024 bash graphics-build/render.sh >/dev/null 2>&1
ls outputs/.cache/parts/*.mov | wc -l | xargs -I{} echo "  {}/24 present"
...
open("/tmp/ln.txt","w").write(f"loudnorm=I=-14:TP=-1.0:LRA=11:measured_I={d['input_i']}:measured_TP={d['input_tp']}"
...
echo "### final"
ffprobe -v error -show_entries stream=codec_type,nb_frames -show_entries format=duration -of default=noprint_wrappers=1 outputs/still-data-engineer-final-longform-2160p.mp4 | tr '\n' ' '; echo
echo "### done"
