...
def main():
    ...
    ap.add_argument("--margin-x", type=float, default=0.0,
                    help="canvas px of side margin that must stay free of ink")
    ap.add_argument("--band-y", type=float, default=0.0,
                    help="canvas px at top and bottom that must stay free of ink")
    ap.add_argument("--canvas", default="", help="WxH canvas, e.g. 1080x1350")
    ap.add_argument("--scale", type=float, default=2.0,
                    help="render px per canvas px, so measurements print in canvas units")
    ...
    for p in parts:
        ...
        c = lambda v: round(v / a.scale)    # render px -> canvas px
        ...
        if a.canvas and (a.margin_x or a.band_y):
            CW, CH = (float(v) for v in a.canvas.lower().split("x"))
            if a.margin_x and (c(x0) < a.margin_x or c(x1) > CW - a.margin_x):
                notes.append(f"crosses the {a.margin_x:.0f}px side margin "
                             f"({c(x0)} -> {c(x1)} against {a.margin_x:.0f}-{CW - a.margin_x:.0f})")
            if a.band_y and (c(y0) < a.band_y or c(y1) > CH - a.band_y):
                notes.append(f"enters the {a.band_y:.0f}px player band "
                             f"({c(y0)} -> {c(y1)} against {a.band_y:.0f}-{CH - a.band_y:.0f})")
        if empty:
