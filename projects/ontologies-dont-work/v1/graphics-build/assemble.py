...
def main():
    ...
    for p, f in zip(parts, files):
        ...
        total_want += want
    print(f"gate 1-2  7 parts present and frame-exact, {total_want} frames "
    ...
    for line in open(md5):
        ...
        prev = h
    pct = 100 * dup / tot
    print(f"\ngate 4    duplicate frames {dup}/{tot} = {pct:.2f}%  (fail above {a.dup_fail_pct}%)")
    if pct > a.dup_fail_pct:
        bad.append(f"duplicate frames {pct:.2f}% exceeds {a.dup_fail_pct}%")
